        ...
        if let tableView = tableView {
            ...
            if self.direction == .down {
                ...
                UIView.animate(withDuration: 0.2, animations: { [unowned self] in
                    self.tableView?.frame = CGRect(x: frame.origin.x + 2, y: (frame.origin.y - tableHeight - self.tableYOffset), width: frame.size.width - 4, height: tableHeight)
                    self.shadowView?.frame = CGRect(x: frame.origin.x + 3, y: (frame.origin.y + 3 - self.tableYOffset), width: frame.size.width - 6, height: 1)
                })
            }
